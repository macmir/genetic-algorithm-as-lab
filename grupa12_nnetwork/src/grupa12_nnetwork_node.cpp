...
Grupa12NnetworkNode::Grupa12NnetworkNode(const rclcpp::NodeOptions & options)
:  Node("grupa12_nnetwork", options), neural_network_(3, 2, 2)
{
  ...
  grupa12_nnetwork_ = std::make_unique<grupa12_nnetwork::Grupa12Nnetwork>();
  ...
  lidar_means_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
    ...
    std::bind(&Grupa12NnetworkNode::lidarMeansCallback, this, std::placeholders::_1));

  localization_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
    "/localization/cartographer/pose", 
    custom_qos, // QoS profile: Adjust the queue size as needed
    std::bind(&Grupa12NnetworkNode::localizationMeansCallback, this, std::placeholders::_1));

  collision_detected_sub_ = this->create_subscription<std_msgs::msg::Bool>(
      "/collision_detected", custom_qos, std::bind(&Grupa12NnetworkNode::collisionDetectedCallback, this, std::placeholders::_1));

  elapsed_time_sub_ = this->create_subscription<diagnostic_msgs::msg::DiagnosticArray>(
      "/diagnostics", custom_qos, std::bind(&Grupa12NnetworkNode::elapsedTimeCallback, this, std::placeholders::_1));
  ...
  grupa12_nnetwork_->foo(param_name_);
}
void Grupa12NnetworkNode::lidarMeansCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
...
void Grupa12NnetworkNode::localizationMeansCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
    // Process the incoming lidar means data
    // For example, log the size of the received vector
...
void Grupa12NnetworkNode::collisionDetectedCallback(std_msgs::msg::Bool::SharedPtr msg)
{
    ...
    if (msg->data==1)
    {
      ...
      if(genetic_networks.size() == 25)
      {
        ...
        double mean_fitness = calculateMeanFitness(genetic_networks);
      ...
      else
      {
        if(is_first_generation == 1)
        {
          ...
          float time_result = std::stof(total_time)+std::stof(lap_timer)-std::stof(last_total_time);
...
void Grupa12NnetworkNode::elapsedTimeCallback(diagnostic_msgs::msg::DiagnosticArray::SharedPtr msg)
{
...
SimpleNeuralNetwork Grupa12NnetworkNode::crossover(const SimpleNeuralNetwork& parent1, const SimpleNeuralNetwork& parent2)
{
...
void Grupa12NnetworkNode::mutate(SimpleNeuralNetwork& nn, double mutation_rate)
{
...
double Grupa12NnetworkNode::calculateMeanFitness(const std::vector<SimpleNeuralNetwork>& networks)
{
  if (networks.empty()) {
...
SimpleNeuralNetwork Grupa12NnetworkNode::selectParent(const std::vector<SimpleNeuralNetwork>& networks, double total_fitness)
{
    std::random_device rd;
    std::mt19937 gen(rd());
...
RCLCPP_COMPONENTS_REGISTER_NODE(grupa12_nnetwork::Grupa12NnetworkNode)
